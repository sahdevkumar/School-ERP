...
export const Admission: React.FC<AdmissionProps> = ({ initialSearch }) => {
  ...
  React.useEffect(() => {
    ...
    const loadSettings = async () => {
      const { classes, sections } = await dbService.getClassesAndSections();
  ...
  const handleEditorSave = async (blob: Blob) => {
    ...
    if (result.publicUrl) {
       ...
       showToast("Failed to upload photo: " + errorMsg, 'error');
    }
  };
  ...
  const handleDocumentUpload = async (docType: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast("An unexpected error occurred: " + errorMsg, 'error');
    } finally {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'string' ? result.error : JSON.stringify(result.error);
      showToast("Failed to update profile: " + errorMsg, 'error');
    }
    setIsSaving(false);
  };
  ...
  const handleFinalizeAdmission = async () => {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'string' ? result.error : JSON.stringify(result.error);
      showToast("Admission finalization failed: " + errorMsg, 'error');
    }
    setIsSaving(false);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto" onClick={() => { setShowColumnMenu(false); setShowExportMenu(false); }}>
          ...
          <div className="bg-white dark:bg-gray-800 w-full h-full md:h-[90vh] md:max-w-5xl md:rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-scale-in">
            ...
            <div className="w-full md:w-64 bg-gray-50 dark:bg-gray-900/50 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 flex flex-col shrink-0 md:h-full">
              <div className="p-3 md:p-6 flex flex-row md:flex-col items-center gap-3 md:gap-0 border-b border-gray-200 dark:border-gray-700 md:border-b-0 shrink-0">
                ...
                <button onClick={() => setProfileModalOpen(false)} className="md:hidden ml-auto p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              
              {/* Navigation with hidden text on mobile */}
              <nav className="flex md:flex-col overflow-x-auto md:overflow-y-auto p-2 md:p-4 gap-2 border-b md:border-b-0 border-gray-200 dark:border-gray-700 shrink-0 md:flex-1 min-h-0">
                {[{ id: 'personal', label: 'Personal', icon: User }, { id: 'academic', label: 'Academic', icon: BookOpen }, { id: 'parents', label: 'Parents', icon: Users }, { id: 'facilities', label: 'Facilities', icon: Bus }, { id: 'documents', label: 'Documents', icon: FileText }].map((tab) => (
                  <button 
                    ...
                    className={`flex-shrink-0 flex items-center justify-center md:justify-start gap-2 px-3 py-2 md:py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${activeTab === tab.id ? 'bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 shadow-sm border border-gray-200 dark:border-gray-700' : 'text-gray-600 dark:text-gray-400 hover:bg-white/50 dark:hover:bg-gray-800/50'}`}
                  >
              ...
              {formData.student_status === 'provisional' && (
                <div className="p-4 mt-auto border-t border-gray-200 dark:border-gray-700 hidden md:block">
                  <button onClick={() => setConfirmationModalOpen(true)} className="w-full px-4 py-2 bg-green-600 text-white font-medium rounded-lg flex items-center justify-center gap-2 hover:bg-green-700 transition-colors shadow-lg shadow-green-500/20">
                    <CheckCircle className="w-5 h-5" /> Approve Admission
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden min-h-0">
              <div className="hidden md:flex p-4 border-b border-gray-200 dark:border-gray-700 justify-between items-center bg-white dark:bg-gray-800">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setProfileModalOpen(false)} className="p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-white dark:bg-gray-800">
                <form id="profile-form" onSubmit={handleSaveProfile} className="space-y-6">
                  {/* ... (Existing form fields content remains exactly the same) ... */}
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex flex-col md:flex-row justify-end gap-3 shrink-0 pb-6 md:pb-4 z-10 relative">
                {formData.student_status === 'provisional' && (
                  <button onClick={() => setConfirmationModalOpen(true)} className="md:hidden w-full px-4 py-2 bg-green-600 text-white font-medium rounded-lg flex items-center justify-center gap-2 hover:bg-green-700 transition-colors shadow-lg shadow-green-500/20 mb-2 md:mb-0">
                    <CheckCircle className="w-5 h-5" /> Approve Admission
                  </button>
                )}
                <button onClick={() => setProfileModalOpen(false)} className="md:hidden px-6 py-2.5 text-gray-700 font-medium hover:bg-gray-200 rounded-xl bg-gray-100 dark:bg-gray-700 dark:text-gray-300">Close</button>
                <button onClick={handleSaveProfile} disabled={isSaving} className="px-8 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20">{isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />} Save Changes</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Other Modals (Success, Confirmation, Import, Editor) */}
