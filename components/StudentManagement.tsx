...
import { 
  ...
  Table as TableIcon, UserX, UserCheck, AlertTriangle, Filter
} from 'lucide-react';
...
export const StudentManagement: React.FC<StudentManagementProps> = ({ initialStudentSearch, pageTitle = "Student Management" }) => {
  ...
  // ... (keeping other handlers same: handleInputChange, handleSameAsMobileChange, handlePhotoSelect, etc.)
  ...
  const renderDocumentCard = (docType: string) => {
    ...
    return (
      <div className={`border-2 border-dashed ${docUrl ? 'border-green-500 bg-green-50 dark:bg-green-900/10' : 'border-gray-300 dark:border-gray-600'} p-6 text-center rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 relative transition-colors group`}>
        {/* Only enable input if edit permission exists */}
        {can('students', 'edit') && (
          <input 
            type="file" 
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto" onClick={() => { setShowColumnMenu(false); setShowExportMenu(false); }}>
      {/* ... Header and View Mode code ... (Same as before, just adding permission checks where needed) */}
      <div>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col lg:flex-row gap-4 justify-between items-center">
         ...
         <div className="flex items-center gap-2 flex-wrap justify-end w-full lg:w-auto">
            ...
            {/* Note: In read-only view, view buttons are still accessible, but actions inside profile are disabled */}
         </div>
      </div>
      ...
      {isLoading ? (<div className="flex justify-center py-12"><Loader2 className="w-10 h-10 animate-spin text-indigo-600" /></div>) : 
        ...
        /* Render View Modes (Grid/List/Thumb) - mostly same, just updating Action buttons */
        viewMode === 'list' ? (
                            ...
                            {visibleColumns.action && (
                              <td className="px-6 py-4 text-right align-top">
                                 <div className="flex items-center justify-end gap-2">
                                   ...
                                   {can('students', 'delete') && (
                                     <button onClick={(e) => { e.stopPropagation(); initiateStatusToggle(student); }} className={`p-1.5 rounded-lg transition-colors ${showInactive ? 'text-green-600 hover:bg-green-50' : 'text-red-500 hover:bg-red-50'}`} title={showInactive ? 'Re-activate' : 'Mark Inactive'}>
          ...
          /* Grid/Thumbnail views (omitted for brevity, essentially just wrapped similarly) */
            ...
            <div className="w-full md:w-64 bg-gray-50 dark:bg-gray-900/50 border-r border-gray-200 dark:border-gray-700 flex flex-col shrink-0">
               ...
               {can('students', 'delete') && (
                 <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                    <button type="button" onClick={() => initiateStatusToggle(formData)} className={`w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors border ${showInactive ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'}`}>
                      {showInactive ? <UserCheck className="w-4 h-4" /> : <UserX className="w-4 h-4" />}
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
               <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-gray-800">
                  <form id="profile-form" onSubmit={handleSaveProfile} className="space-y-6">
                    {/* ... Form Fields (ReadOnly if !canEdit) ... */}
                    {/* For brevity, applying logic to wrapper form field set or specific inputs */}
                    <fieldset disabled={!can('students', 'edit')} className="contents">
                       {/* ... Tab Content from previous implementation ... */}
                       {activeTab === 'personal' && (
      ...
      {statusToggleModalOpen && studentToToggle && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 text-center">
             ...
             <p className="text-gray-500 mb-6">Are you sure you want to {studentToToggle.student_status === 'active' ? 'deactivate' : 'activate'} this student?</p>
             <div className="flex gap-3 justify-center">
                <button onClick={() => setStatusToggleModalOpen(false)} className="px-5 py-2.5 border rounded-xl">Cancel</button>
      ...
      <style>{`
        ...
        fieldset[disabled] .input-field { background-color: #f3f4f6; color: #9ca3af; cursor: not-allowed; }
        .dark fieldset[disabled] .input-field { background-color: #374151; color: #9ca3af; }
      `}</style>
    </div>
  );
}
